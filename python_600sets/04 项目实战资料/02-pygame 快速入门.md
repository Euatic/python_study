...
<img src="media/15025046487919/001_pygame%E7%9A%84init%E5%92%8Cquit.png" alt="001_pygame的init和quit-w2545" style="zoom:67%;" />
...
* **作用** —— 创建游戏显示窗口
* **参数**
    * `resolution` 指定屏幕的 `宽` 和 `高`，默认创建的窗口大小和屏幕大小一致——**重要参数**
    * `flags` 参数指定屏幕的附加选项，例如是否全屏等等，默认不需要传递
    * `depth` 参数表示**颜色**的位数，默认自动匹配
* **返回值**
...
* 要在屏幕上 **看到某一个图像的内容**，需要按照三个步骤：
    ...
    2. 使用 **游戏屏幕** 对象，调用 `blit` 方法 将图像绘制到指定位置（元组）
...
* 调用 **精灵组** 对象的 `update()` 方法
    * 可以 **自动** 调用 **组内每一个精灵** 调用各自的 `update()` 方法
...
* 精灵组
    * 包含 **多个** **精灵对象**
    * `update` 方法，让精灵组中的所有精灵调用 `update` 方法更新位置 —— 让组中的所有精灵更新位置
